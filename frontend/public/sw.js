// Renzo service worker (Next.js static-export shell).
...
// Cache names are shared with the previous frontend on purpose: renzo-offline-v1
// holds user downloads and must survive the UI swap.
const SHELL = "renzo-shell-v1";
const OFFLINE = "renzo-offline-v1";
const CORE = [
  "/", "/library/", "/updates/", "/history/", "/downloads/",
  "/title/", "/watch/", "/settings/",
  "/tvnav.js", "/site.webmanifest", "/renzo-banner.png",
...
self.addEventListener("fetch", (e) => {
  ...
  // Navigations: network-first (fresh UI); offline, serve that route's cached
  // HTML if we have it, else the shell root (client boot handles the rest).
  ...
  // Shell assets (Next chunks, RSC payloads, icons — with or without a query):
  // stale-while-revalidate, populated organically as the app is used.
  e.respondWith((async () => {
